const validateModal=document.querySelector('.bg-validate-modal');const validate=document.querySelector('.validate-game');const validateContent=document.querySelector('.validate-modal-content')
const validateClose=document.querySelector('.validate-modal-close');const sessionName=document.querySelector('#sessionUser')
...
validate.addEventListener('click',function(){if(sessionState.innerHTML==='True'){validateModal.style.display='block'
setTimeout(()=>{validateContent.style.transform='translate(0rem, 0rem)'},100)}
else{login.style.display='block'}});validateClose.addEventListener('click',function(){validateContent.style.transform='translate(0rem, -5rem)'
setTimeout(()=>{validateModal.style.display='none';},100)});loginClose.addEventListener('click',()=>{login.style.display='none'})
try{notifyClose.addEventListener('click',()=>{notifyDiv.style.right='-100%';setTimeout(function(){notifyDiv.style.display='none'},500)})}catch(e){}
var url=window.location.pathname;var gamename=url.substring(url.lastIndexOf('/')+1);$(document).on('submit','.validate-modal-form',function(e){e.preventDefault();var formData={'gameid':$('input[name=gameid]').val(),'csrfmiddlewaretoken':$('input[name=csrfmiddlewaretoken]').val()}
$.ajax({type:'POST',url:'/game/'+gamename,data:formData,}).done(function(response_data){validateModal.style.display='none';setTimeout(function(){validate.style.cursor='default'
...
document.querySelector('.validate-game').style.pointerEvents='none'},200);})});const sideBar=document.querySelector('.sideBar');const mobNav=document.querySelector('.mobNav');const container=document.querySelector('.container');const carousel=document.querySelector('.carousel');const body=document.getElementsByTagName('body');const lowerLogin=document.querySelector('.lower-sec-login');const lowerProfile=document.querySelector('.lower-sec-profile');const session=document.querySelector('#session');const sessionUser=document.querySelector('#sessionUser');const notifyDiv=document.querySelector('.notify-div');const notifyHead=document.querySelector('.notify-heading-content');const notifyMessage=document.querySelector('.notify-message')
