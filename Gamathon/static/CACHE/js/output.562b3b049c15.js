...
const options={treshold:0,rootMargin:"0px 0px 0px 0px"}
const objects=document.querySelectorAll('.anim')
observer=new IntersectionObserver((entries)=>{entries.forEach(entry=>{if(entry.isIntersecting){entry.target.style.animation=`anim1 1s ${entry.target.dataset.delay} forwards ease-out`}
else{entry.target.style.animation='none'}})},options)
