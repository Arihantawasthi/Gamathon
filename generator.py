...
group = Round.objects.get(tour=tour, stage=Stage.objects.get(stage_name='Qualifiers', tour=tour), round_name='Group 9')
...
def matchGenerator(scs_all):
#def matchGenerator():
    with open (f'../tour_info/RisingEra/{group.round_name}.csv', 'r') as f:
        reader = csv.reader(f)
        next(reader)
        for r in reader:
           ...
           #For Teams
            for i in s:
                if i.team != None:
                    ...
                    print(f'            {i.solo}                   ')
                    try:
                        o = ScoreCard.objects.filter(tour=tour, match=Match.objects.get(match_name='Overall', round_id=group, tour=tour), solo=i.solo)[0]
                    except:
                        print(f'          {i.solo} not found.          ')
                        continue
                o.kills += i.kills
